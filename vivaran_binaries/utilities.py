...
def drop_col_all_nans(data_df):
    ...
    data_df_acc = deepcopy(data_df[data_df["label"] == 1])
...
def preprocess_data(data_df, progname):
    ...
    ##########################################################################
    # data_df_types = data_df.drop(columns=["result"]).loc[0, :].values.tolist()
    # data_df_types = []
    # for i in boolean:
    #     data_df_types.append("Bool")
    # for i in integer:
    #     data_df_types.append("Int")
    ...
    #     elif data_df_types[idx] in ["Bool"]:
    #         types_dict["Bool"].append(col)
