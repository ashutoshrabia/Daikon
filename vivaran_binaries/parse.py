...
# def split_cicular_implications(implications):
#     """Now if a == 1 ==> a == b then we convert this implication to a == 1 ==> b == 1
#     (if a == 1 ==> b == 1 is not present in set of all implications)
#     Similarly if a == 0 ==> a == b then we convert this implication to a == 0 ==> b == 0
#     """
#     new_imp = []
#     for imply in implications:
#         ante, cons = imply.split("==>")[0], imply.split("==>")[1]
#         if "== 1" in ante:
#             ante_var = ante.split("==")[0].strip().strip("(")
#             if f"== {ante_var}" in cons:
#                 cons_var = cons.split("==")[0].strip().strip("(")
#                 if f"{ante} ==> ({cons_var} == 1)" not in new_imp:
#                     new_imp.append(f"{ante} ==> ({cons_var} == 1)")
#             else:
#                 new_imp.append(imply)
#         elif "== 0" in ante:
#             ante_var = ante.split("==")[0].strip().strip("(")
#             if f"{ante_var} ==" in cons:
#                 cons_var = cons.split("==")[1].strip().strip(")")
#                 if f"{ante} ==> ({cons_var} == 0)" not in new_imp:
#                     new_imp.append(f"{ante} ==> ({cons_var} == 0)")
#             else:
#                 new_imp.append(imply)
#         else:
#             new_imp.append(imply)
    ...
    for key in mapping.keys():
        ...
        for split in n_sp:
            if split in mapping[key] and key not in modified_map.keys():
                modified_map[key] = split
                break
    # m_keys = list(modified_map.keys())
    # for key in m_keys:
    #     if "(not(" in modified_map[key]:
    #         if "== true" in key:
    #             modified_map[key.replace("== true", "== false")] = (
    #                 modified_map[key].replace("(not(", "(").replace("))", ")")
    #             )
    #         elif "== false" in key:
    #             modified_map[key.replace("== false", "== true")] = (
    #                 modified_map[key].replace("(not(", "(").replace("))", ")")
    #             )
    return modified_map
...
#     neg_split_points = []
#     split_points = []
#     with open(f"{PATH}/accepted/{url}.spinfo", "r") as f:
#         s_p = f.readlines()
#     s_p = [ele.strip() for ele in s_p[1:]]
#     for ele in s_p:
#         if "==" in ele or "&&" in ele:
