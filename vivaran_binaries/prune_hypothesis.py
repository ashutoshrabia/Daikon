...
# def old_get_sat_model(mid, boundary, s_plus, other_constr, sel_constr, ope_types_dict):
#     ante_constr = antecedent_constr(boundary, ope_types_dict)
#     var = get_typed_exp(boundary["var"], ope_types_dict)
#     all_constr = s_plus + ante_constr + [var == mid] + sel_constr + other_constr
#     res = z3.unsat
#     s = z3.Solver()
#     while res == z3.unsat:
#         s.reset()
#         s.set(unsat_core=True)
#         cons_d = {f"a{idx}": ele for idx, ele in enumerate(all_constr)}
...
#                 if value not in (ante_constr + [var == mid] + sel_constr)
#             }
#             all_constr = [ele for ele in all_constr if ele not in unsat_dict.values()]
#             print(f'{all_constr=}')
#             # print(f"len_of_splus::{len(all_constr)}")
