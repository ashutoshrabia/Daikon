...
def gen_new_c(func_name, params, ret_spec):
    ...
    lines.append(f"    if (p1Null) p1 = NULL;")
    lines.append(f"    else if (isValidp1) {{")
    lines.append(f"        p1 = (char*)malloc(sizeof1);")
    lines.append(f"        memset(p1, 'A', sizeof1);")
    lines.append(f"        if (isStringp1) p1[strlen1] = '\\0';")
    lines.append("    }")
    lines.append("")
    if "p2" in ptrs:
        lines.append(f"    if (p2Null) p2 = NULL;")
        lines.append(f"    else if (isValidp2 && isDisjointp1p2) {{")
        lines.append(f"        p2 = (char*)malloc(sizeof2);")
        lines.append(f"        memset(p2, 'A', sizeof2);")
        lines.append(f"        if (isStringp2) p2[strlen2] = '\\0';")
        lines.append("    }")
        lines.append("")

    # disjoint aliasing
    for a, b in combinations(ptrs, 2):
        lines.append(f"    if (!isDisjoint{a}{b} && !{a}Null && !{b}Null) {{")
    ...
    for a, b in combinations(ptrs, 2):
        lines.append(f"    if (!{b}Null && isDisjoint{a}{b} && isValid{b}) free({b});")
