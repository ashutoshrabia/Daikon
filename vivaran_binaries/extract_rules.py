...
#         if ele in ope_types_dict["Bool"] or ele in ope_types_dict["Select"]
#     ]
#     ne_date_cols = [
#         ele for ele in df.columns if ele[ele.find("_") + 1 :] in ope_types_dict["Date"]
#     ]
...
def create_split_info(sel_dict, ope_types_dict, url, iterations):
    ...
    if "p2Null" in check_sel_cols:
        extra_splits = [
            "isDisjointp1p2 && (isStringp2==0)",
            "isDisjointp1p2 && isStringp1",
            "isStringp2 && isDisjointp1p2",
            "(isDisjointp1p2 == 0) && isStringp1",
            "isDisjointp1p2 && (isStringp1==0)",
            "(p1Null==0) && isValidp1 && isStringp1",
            "(p2Null==0) && isValidp2 && isStringp2",
            "(p2Null==0) && isValidp2",
        ]
        for s in extra_splits:
            if s not in check_splits:
                check_splits.append(s)
