...
# sel_dict = {'status': ['Open', 'Working', 'Pending Review', 'Overdue', 'Template', 'Completed', 'Cancelled'], 'priority': ['Low', 'Medium', 'High', 'Urgent']}
# orig_types_dict = {'subject': 'Data', 'project': 'Link', 'issue': 'Link', 'type': 'Link',
#              'color': 'Color', 'is_group': 'Check', 'is_template': 'Check',
#              'status': 'Select', 'priority': 'Select', 'task_weight': 'Float',
#              'parent_task': 'Link', 'completed_by': 'Link', 'completed_on': 'Date',
#              'exp_start_date': 'Date', 'expected_time': 'Float', 'start': 'Int',
#              'exp_end_date': 'Date', 'progress': 'Percent', 'duration': 'Int',
#              'is_milestone': 'Check', 'description': 'Text Editor', 'depends_on': 'Table',
#              'depends_on_tasks': 'Code', 'review_date': 'Date', 'closing_date': 'Date',
#              'department': 'Link', 'company': 'Link'}

# counter_ex_df = pd.read_csv(f"{PATH}/debug_data/counter_df0.csv")
# counter_to_data(counter_ex_df, orig_types_dict, sel_dict)
